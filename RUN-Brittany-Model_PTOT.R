...
#LOAD DATA FOR MODEL AND MAKE PLSR MODEL BASED ON WHAT WE KNOW ABOUT WHATEVER
          ...
          #read the data specified by the vector filename
            pTot<-11
            myData<-loadDataFile(path,ModelFilename[bestFitFiles[pTot]])
...
#LOAD DATA FROM BRITTANY FOR PROJECTION
            data0<-read.table(file=paste(path,filename[1],sep=""),sep="\t",header=TRUE,skip=0)
            data1<-read.table(file=paste(path,filename[2],sep=""),sep="\t",header=TRUE,skip=0)
            data2<-read.table(file=paste(path,filename[3],sep=""),sep="\t",header=TRUE,skip=0)
            data3<-read.table(file=paste(path,filename[4],sep=""),sep="\t",header=TRUE,skip=0)
...
write.table(c(as.POSIXct(realTime, origin="1970-01-01",tz="UTC"),Predict),
                file=pTotFile, append = FALSE,row.names=FALSE,col.names=c("realTime","Predicted"))


#WRITE ANOTHER FILE WITH REALTIME, OBSERVED AND PREDICTED VALUES
pTotModel$ObservedAndPredicted[,3]<-as.POSIXct(pTotModel$ObservedAndPredicted[,3], origin="1970-01-01",tz="UTC")
pTotFile<-paste(path,"pTot_observedAndPredicted.txt",sep="")
write.table(pTotModel$ObservedAndPredicted[,3],file=pTotFile, append = FALSE,row.names=FALSE,col.names=FALSE)

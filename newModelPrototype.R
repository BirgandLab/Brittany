...
startDates<-c(as.POSIXct(paste("2010-10-01 00:00:00",sep=""),tz="UTC"),
             ...
             as.POSIXct(paste("2013-10-01 00:00:00",sep=""),tz="UTC"),
        ...
        for(chemical in 1:9){
                  ...
                  #numComp<-4    #specify number of components to use in the model
                      for(k in 1:length(Types)){
                          ...
                          iD[counter,1]<-Chem[chemN[chemical]]
                                  ...
                                  modelOutput<-modelExecution(chemical,numComp,subsetRate,calibration,specDataToModel,fitEval,fitFile,fitFileOut)
                                        ...
                                        jpeg(file = paste(OutputPath,
                                                    ...
                                                    plot(as.POSIXct(flow$realTime,origin="1970-01-01 00:00:00"),
                                                         flow$flow,col="green",type="l",xaxt="n",yaxt="n",xlab="",ylab="",
                                                         xlim=c(min(PredictedAnnualConcentrationTS$realTime),max(PredictedAnnualConcentrationTS$realTime)),
                                                    )
                                                    par(new=TRUE)
                                                    plot(as.POSIXct(PredictedAnnualConcentrationTS$realTime,origin="1970-01-01 00:00:00"),
                                                          PredictedAnnualConcentrationTS$concentration,type="l",col=rainbow(10)[i],
                                                          main=chemicals[chemical],
                                                          ylim=c(min(PredictedAnnualConcentrationTS$concentration),max(PredictedAnnualConcentrationTS$concentration)),
                                                          xlim=c(min(PredictedAnnualConcentrationTS$realTime),max(PredictedAnnualConcentrationTS$realTime)),
                                                          xlab="date",

...
n<-calibPoints[awesome]
            ...
            for(i in 1:j){   
              ...
              jpeg(file = paste("C:/Users/FBlab/Desktop/work_here/Data/","MES_",n,"_",sprintf("%03d.jpg",i),sep=""),width=800,height=400)

              par(mfrow=c(1,2))
                                    ModelNSE<-NSE(sim=TurbModelRandom$ObservedAndPredicted[,2],obs=TurbModelRandom$ObservedAndPredicted[,1])
                                    plot(TurbModelRandom$ObservedAndPredicted[,2],TurbModelRandom$ObservedAndPredicted[,1],xlim=xlimits,ylim=ylimits,
                                         xlab="predicted", ylab="observed",main=paste(n," pt calibration",sep=""),
                                         cex.lab=1.7,
                                         cex.main=2
                                    )
                                    abline(0,1,lwd=2)
                                    text(50,150,labels=paste("NSE =",round(ModelNSE,3),sep=""),cex=2)
                          ...
                          #calculate NSE for whole dataset
                                    ...
                                    text(50,150,labels=paste("NSE =",round(DataNSE,3),sep=""),cex=2)
               dev.off()                     

...
for (i in 1:3){
  ...
  stopDate<-as.POSIXct(paste(stopYear[i],"-08-01 00:00:00",sep=""),tz="gmt")
            ...
            numComp<-numberOfComponentsToUse(useUsFP,useUsChem$PPO43)
            
            #numComp<-25
            #pTotModel<-PLSRFitAndTest(myData$fingerPrints,myData$ChemData$Turb,myData$realTime,numComp,fitEval,fitFile,fitFileOut,0)
            PPO43Model<-PLSRFitAndTest(useUsFP,useUsChem$PPO43,useUsRealTIme,numComp,fitEval,fitFile,fitFileOut,0)
...
Predict<-predict(PPO43Model$Fit,data.matrix(useUsfp),ncomp=numComp,type=c("response"))
...
outFile<-paste(path,"PPO43_projected",startYear[i],stopYear[i],".txt",sep="")
...
plot(projection,type="l",col="black",main="projected PPO43",xlab="date",ylab="concentration",ylim=c(-40,600))

#WRITE ANOTHER FILE WITH REALTIME, OBSERVED AND PREDICTED VALUES
model<-as.data.frame(matrix(0,nrow=length(PPO43Model$ObservedAndPredicted[,3]),ncol=3))
model[,1]<-as.POSIXct(PPO43Model$ObservedAndPredicted[,3], origin="1970-01-01",tz="")
model[,2]<-round((PPO43Model$ObservedAndPredicted[,1]),5)
model[,3]<-round((PPO43Model$ObservedAndPredicted[,2]),5)
if(i==1){
...
outFile<-paste(path,"PPO43_observedAndPredicted",startYear[i],stopYear[i],".txt",sep="")
...
outFile<-paste(path,"PPO43_projected_wholeDataSet.txt",sep="")

...
for (i in 1:3){
  ...
  stopDate<-as.POSIXct(paste(stopYear[i],"-08-01 00:00:00",sep=""),tz="gmt")
            ...
            numComp<-numberOfComponentsToUse(useUsFP,useUsChem$NO3)
            
            #numComp<-25
            #pTotModel<-PLSRFitAndTest(myData$fingerPrints,myData$ChemData$Turb,myData$realTime,numComp,fitEval,fitFile,fitFileOut,0)
            NO3Model<-PLSRFitAndTest(useUsFP,useUsChem$NO3,useUsRealTIme,numComp,fitEval,fitFile,fitFileOut,0)
...
Predict<-predict(NO3Model$Fit,data.matrix(useUsfp),ncomp=numComp,type=c("response"))
...
outFile<-paste(path,"NO3_projected",startYear[i],stopYear[i],".txt",sep="")
...
plot(projection,type="l",col="black",main="projected NO3",xlab="date",ylab="concentration",ylim=c(-40,600))

#WRITE ANOTHER FILE WITH REALTIME, OBSERVED AND PREDICTED VALUES
model<-as.data.frame(matrix(0,nrow=length(NO3Model$ObservedAndPredicted[,3]),ncol=3))
model[,1]<-as.POSIXct(NO3Model$ObservedAndPredicted[,3], origin="1970-01-01",tz="")
model[,2]<-round((NO3Model$ObservedAndPredicted[,1]),5)
model[,3]<-round((NO3Model$ObservedAndPredicted[,2]),5)
if(i==1){
...
outFile<-paste(path,"NO3_observedAndPredicted",startYear[i],stopYear[i],".txt",sep="")
...
outFile<-paste(path,"NO3_projected_wholeDataSet.txt",sep="")
...
outFile<-paste(path,"NO3_observedAndPredicted_wholeDataSet.txt",sep="")

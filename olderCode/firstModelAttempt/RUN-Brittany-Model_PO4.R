...
#LOAD DATA FOR MODEL AND MAKE PLSR MODEL BASED ON WHAT WE KNOW ABOUT WHATEVER
          ...
          #read the data specified by the vector filename
            pTot<-11
            ppo43<-10
            myData<-loadDataFile(path,ModelFilename[bestFitFiles[ppo43]])
            numComp<-numberOfComponentsToUse(myData$fingerPrints,myData$ChemData$PPO43)
            pTotModel<-PLSRFitAndTest(myData$fingerPrints,myData$ChemData$PPO43,myData$realTime,numComp,fitEval,fitFile,fitFileOut,0)
...
pTotFile<-paste(path,"ppo43_projected.txt",sep="")
...
plot(output,type="l",col="black",main="projected PPO43",xlab="date",ylab="concentration",ylim=c(-0.1,1.5))
...
pTotFile<-paste(path,"ppo43_observedAndPredicted.txt",sep="")

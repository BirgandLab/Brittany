...
for (i in 1:3){
  ...
  stopDate<-as.POSIXct(paste(stopYear[i],"-08-01 00:00:00",sep=""),tz="gmt")
            ...
            numComp<-numberOfComponentsToUse(useUsFP,useUsChem$SO4)
            
            #numComp<-25
            #pTotModel<-PLSRFitAndTest(myData$fingerPrints,myData$ChemData$Turb,myData$realTime,numComp,fitEval,fitFile,fitFileOut,0)
            SO4Model<-PLSRFitAndTest(useUsFP,useUsChem$SO4,useUsRealTIme,numComp,fitEval,fitFile,fitFileOut,0)
...
Predict<-predict(SO4Model$Fit,data.matrix(useUsfp),ncomp=numComp,type=c("response"))
...
outFile<-paste(path,"SO4_projected",startYear[i],stopYear[i],".txt",sep="")
...
plot(projection,type="l",col="black",main="projected SO4",xlab="date",ylab="concentration",ylim=c(-.25,0.9))

#WRITE ANOTHER FILE WITH REALTIME, OBSERVED AND PREDICTED VALUES
model<-as.data.frame(matrix(0,nrow=length(SO4Model$ObservedAndPredicted[,3]),ncol=3))
model[,1]<-as.POSIXct(SO4Model$ObservedAndPredicted[,3], origin="1970-01-01",tz="")
model[,2]<-round((SO4Model$ObservedAndPredicted[,1]),5)
model[,3]<-round((SO4Model$ObservedAndPredicted[,2]),5)
if(i==1){
...
outFile<-paste(path,"SO4_observedAndPredicted",startYear[i],stopYear[i],".txt",sep="")
...
plot(projected,type="l",col="black",main="projected SO4 from annual models",xlab="date",ylab="concentration",ylim=c(-.25,0.9))
outFile<-paste(path,"SO4_projected_wholeDataSet.txt",sep="")
...
outFile<-paste(path,"SO4_observedAndPredicted_wholeDataSet.txt",sep="")

...
#LOAD DATA FOR MODEL AND MAKE PLSR MODEL BASED ON WHAT WE KNOW ABOUT WHATEVER
          ...
          #read the data specified by the vector filename
            ...
            numComp<-numberOfComponentsToUse(myData$fingerPrints,myData$ChemData$NO2)
            #numComp<-25
            NO2Model<-PLSRFitAndTest(myData$fingerPrints,myData$ChemData$NO2,myData$realTime,numComp,fitEval,fitFile,fitFileOut,0)
...
#USE THE MODEL TO PREDICT OUTPUT FOR THE WHOLE TIME PERIOD
            ...
            Predict<-predict(NO2Model$Fit,data.matrix(fingerPrints),ncomp=numComp,type=c("response"))
...
pTotFile<-paste(path,"NO2_projected.txt",sep="")
...
output<-as.data.frame(matrix(0,nrow=length(NO2Model$ObservedAndPredicted[,3]),ncol=3))
output[,1]<-as.POSIXct(NO2Model$ObservedAndPredicted[,3], origin="1970-01-01",tz="")
output[,2]<-round((NO2Model$ObservedAndPredicted[,1]),5)
output[,3]<-round((NO2Model$ObservedAndPredicted[,2]),5)


pTotFile<-paste(path,"NO2_observedAndPredicted.txt",sep="")

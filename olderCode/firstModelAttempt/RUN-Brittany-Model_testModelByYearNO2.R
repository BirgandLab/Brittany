...
Predict<-predict(NO2Model$Fit,data.matrix(useUsfp),ncomp=numComp,type=c("response"))
...
outFile<-paste(path,"NO2_projected",startYear[i],stopYear[i],".txt",sep="")
...
plot(projection,type="l",col="black",main="projected NO2",xlab="date",ylab="concentration",ylim=c(-.25,0.9))

#WRITE ANOTHER FILE WITH REALTIME, OBSERVED AND PREDICTED VALUES
model<-as.data.frame(matrix(0,nrow=length(NO2Model$ObservedAndPredicted[,3]),ncol=3))
model[,1]<-as.POSIXct(NO2Model$ObservedAndPredicted[,3], origin="1970-01-01",tz="")
model[,2]<-round((NO2Model$ObservedAndPredicted[,1]),5)
model[,3]<-round((NO2Model$ObservedAndPredicted[,2]),5)
if(i==1){
...
outFile<-paste(path,"NO2_observedAndPredicted",startYear[i],stopYear[i],".txt",sep="")
...
plot(projected,type="l",col="black",main="projected NO2 from annual models",xlab="date",ylab="concentration",ylim=c(-.25,0.9))
outFile<-paste(path,"NO2_projected_wholeDataSet.txt",sep="")
...
outFile<-paste(path,"NO2_observedAndPredicted_wholeDataSet.txt",sep="")

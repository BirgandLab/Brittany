...
flowApprox<-approx(uuFlowTime,uuFlowFlow,n=totmin+1) #create a time series of flow at 1 minute intervals for the whole dataset
        ...
        CHEM<-5
        j<-50 #the number of times to run through each model style
        calibPoints=c(12,24,52,104,422) #NUMBER OF CALIBRATION POINTS
        numComp<-5
...
xlimits<-c(min(useUsChem),max(useUsChem))
ylimits<-xlimits
            ...
            for(i in 1:j){   
              par(mfrow=c(1,2))
                          #SUBSET CALIBRATION DATA RANDOMLY
                                ...
                                #USE THE MODEL TO PREDICT concentration for lab fingerprints    
                                      PredictRandom<-predict(TurbModelRandom$Fit,data.matrix(useUsFP),ncomp=numComp,type=c("response"))
                          #calculate NSE for model
              
              jpeg(file = paste("C:/Users/FBlab/Desktop/work_here/Data/","NO3_",n,"_",sprintf("%03d.jpg",i),sep=""),width=800,height=400)

              par(mfrow=c(1,2))
                                    ModelNSE<-NSE(sim=TurbModelRandom$ObservedAndPredicted[,2],obs=TurbModelRandom$ObservedAndPredicted[,1])
                                    plot(TurbModelRandom$ObservedAndPredicted[,2],TurbModelRandom$ObservedAndPredicted[,2],xlim=xlimits,ylim=ylimits,
                                         xlab="predicted", ylab="observed",main=paste(n," pt calibration",sep=""),
                                         cex.lab=1.7,
                                         cex.main=2
                                    )
                                    abline(0,1,lwd=2)
                                    text(9,17,labels=paste("NSE =",round(ModelNSE,3),sep=""),cex=2)
              
                          #calculate NSE for whole dataset
                                    DataNSE<-NSE(sim=PredictRandom[,1,1],obs=useUsChem)
                                    plot(PredictRandom[,1,1],useUsChem,xlim=xlimits,ylim=ylimits,
                                         xlab="predicted", ylab="observed",main="evaluation",
                                         cex.lab=1.7,cex.main=2)
                                    abline(0,1,lwd=2)
                                    abline(lm(useUsChem~PredictRandom[,1,1]),col="red",lwd=3)
                                    text(9,17,labels=paste("NSE =",round(DataNSE,3),sep=""),cex=2)
               dev.off()                     
...
plot(AP_R$x,cumsum(AP_R$y),type="l")
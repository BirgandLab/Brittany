...
#USE THE MODEL TO PREDICT OUTPUT FOR THE WHOLE TIME PERIOD
            ...
            Predict<-predict(MESModel$Fit,data.matrix(fingerPrints),ncomp=numComp,type=c("response"))
...
pTotFile<-paste(path,"MES_projected.txt",sep="")
...
plot(output,type="l",col="black",main="projected MES",xlab="date",ylab="concentration",ylim=c(-20,400))

#WRITE ANOTHER FILE WITH REALTIME, OBSERVED AND PREDICTED VALUES
output<-as.data.frame(matrix(0,nrow=length(MESModel$ObservedAndPredicted[,3]),ncol=3))
output[,1]<-as.POSIXct(MESModel$ObservedAndPredicted[,3], origin="1970-01-01",tz="")
output[,2]<-round((MESModel$ObservedAndPredicted[,1]),5)
output[,3]<-round((MESModel$ObservedAndPredicted[,2]),5)


pTotFile<-paste(path,"MES_observedAndPredicted.txt",sep="")

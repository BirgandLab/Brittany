...
numberOfComponentsToUse<-function(fingerPrint,ChemConc){
...
PLSRFitAndTest<-function(fingerPrint,ChemConc,realTime,numParameters,fitEval,fitFile,fitFileOut,subsample) {
 ...
 Predict<-predict(Fit,data.matrix(fingerPrint),ncomp=numParameters,type=c("response"))
   ...
   if (subsample==0){
  return (list(Fit=Fit,ObsAndPred=ObsAndPred,fitQuality=fitQualityFull,ObservedAndPredicted=op3,Stats=Stats))
   }
  if (subsample>0){
